...
Here, we use stochastic individual-based simulations to identify parameters that affect the probability that a _Z_-linked meiotic drive element will spread and/or cause population suppression. The simulation is heavily inspired by proposals for different types of CRISPR-Cas9 _Z_-linked gene drives laid out by Dr. Kevin Esvelt and colleagues (see [http://www.sculptingevolution.org/genedrives/current/schistosomiasis]()). The 'suppression drives' (i.e. gene drives intended to deplete or remove a population) proposed by Esvelt and colleagues are shown schematically in Figure 1. Firstly, one could add a genetic element (e.g. a casette of genes carrying _Cas9_ and appropriate guide RNAs, promoters, and a phenotypic marker) to the _Z_ chromosome, which targets and cuts repetitive DNA on the W chromosome to create a '_W_-shredder'. Drive-carrying females would then produce few or no daughters, and could pass the drive element to other females via their sons. Esvelt and colleagues point out that the number of offspring produce by drive-carrying females depends on multiple factors, principally the timing of expression the gene drive (e.g. pre- or post-meiosis), the strength of individual-level fitness costs to drive carriers (e.g. due to the expression of _Cas9_), and the ecology of the species (e.g. can lost ova or offspring be easily replaced). They argue that in some cases, drive-carrying females might produce as many (or almost as many) offspring as a wild-type female, only with a male-biased offspring sex ratio (Figure 1, 1A). Alternatively, drive-carrying females might produce half (or less than half) the number of offspring, for example because the drive works by destroying all ova that inherit a W chromosome. Lastly, they also consider a _Z_-linked gene drive that causes carrier females to die or become sterile. They also point out that a single genetic element could be designed so that it also effects gene drive in males, increasing the probability and speed of its invasion. Male gene drive could be accomplished using gene conversion, whereby the gene drive _Z_ converts the non-driving _Z_ in heterozygotes using homiong endonuclease activity followed by DNA repair, leading heterozygous males to prouce almost entirely drive-carrying sperm and offspring. Our simulation was designed to accommodate all of these possibilities.

Impressed by these ideas, we sought to test which parameters of the gene drive, and the ecology of the target species, have the strongest effects on the likelihood and speed with which the gene drive causes extinction. For example, the gene drive will presumably spread faster if it can effect drive in males as well as females, but perhaps a female-only gene drive (which might often be more technically feasible) would be adequate. Also, the population will become more male-biased as the _Z_ drive invades, resulting in eco-evo feedback (REF) that might affect the evolutionary outcome. For example, the altered sex ratio might intensify the fitness advantage accruing to resistant _W_ chromosomes or autosomal modifiers that prevent _W_-shredding, relative to that observed in models of autosome-linked gene drives (REF). Moreover, the change in sex ratio could affect the demographics of the population if males and females differ in their contribution to density dependent population growth, or their migration rates.
