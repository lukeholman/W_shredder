...
Secondly, males with the genotypes Z\*Z+AAbb, Z\*Z+Aabb, or Z\*Z+aabb males produce a fraction $\frac{1}{2}(1 + p_{conv} - p_{conv} p_{nhej})$ of gametes carrying the Z\* allele, $\frac{1}{2}(1 - p_{conv})$ gametes carrying the Z+ allele, and $\frac{1}{2}(p_{conv} p_{nhej}))$ gametes carrying the Zr allele. The parameter $p_{conv}$ represents gene conversion, and when $p_{conv} > 0$, the Z\* allele is over-represented in the gametes of these three male genotypes. The parameter $p_{nhej}$ represents the phenomenon of 'non-homologous end joining', in which a gene drive mediated by an endonuclease

Note that the gamete frequencies produced by Z\*Zr males, or by Z\*Z+ males carrying at least one B allele, conform to the standard Mendelian expectations.

Finally, we model the possibilty that Z\* drive alleles can create resistant Zr alleles in Z\*Z+ individuals, representing the phenomenon of 'non-homologous end joining' (in which a homing endonuclease deletes its target site instead of being copied over to the homologous chromosome). A fraction X of the Z\* drive alleles
