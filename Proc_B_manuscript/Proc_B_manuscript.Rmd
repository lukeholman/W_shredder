...
There are three possible _Z_-linked alleles: a gene drive allele (denoted _Z\*_), a wild-type allele (_Z+_) that is vulnerable to gene drive in _Z\*Z+_ males, and a resistant allele ($Z^r$) that is immune to gene drive in _Z\*_$Z^r$ males. Similarly, there are two possible types of _W_ chromosomes: a wild-type _W_ chromosome (_W+_) that is vulnerable to gene drive by the _Z\*_ allele, and a resistant _W_ chromosome ($W^r$) that is immune to gene drive.
...
Finally, the overall density experienced by female $i$ in patch $j$ ($D_i$) is a composite of the global and local densities given by $D_i = \psi d_g + (1 - \psi)d_j$. The parameter $\psi$ scales the importance of global and local density to female fecundity. When $\psi = 0$, only local density matters and selection on females is entirely "soft", while when $\psi = 1$ only global density matters and selection on females is completely "hard" (similar to the model in [@li2018ev]). Intermediate values of $\psi$ produce a mixture of hard and soft selection on females, and the  pgrowth rate of population depends on density at both scales.
...
The _W_-shredder was able to fail for two principal reasons: it reached fixation without causing extinction, or it went extinct (Table S1). The former happened only if resistance to _W_ shredding could evolve (either via reisistant _W_ chromosomes, or the _A_ allele), while the latter happened whenever the fitness benefits of gene drive were overwhelmed by the fitness costs imposed on drive carriers.
