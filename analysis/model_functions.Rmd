...
combine_results_files <- function(chunk, all_files, 
  ...
  attempt_to_open <- function(file){
    tryCatch(
      ...
      # If it doesn't open, delete the file and print its name
      error = function(cond) {
        print(file)
        unlink(file)
        return(NULL)})
  ...
  print(str(list_of_lists[1:3]))
  
  # Only keep the file names for files that opened properly
  test <- sapply(list_of_lists, function(x) length(x) == 3)
  print(head(test))
