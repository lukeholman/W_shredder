...
set_up_mating_type_tables <- function(parameters, 
  ...
  # I added this section to fix a "bug", but I think it was just an issue with the Spartan server
  # The bug mysteriously fixed itself!! 
...
combine_results_files <- function(cores, wd = getwd()){
  ...
  # Parse the files in manageable chunks:
  all_files <- list.files("data/sim_results", full.names = TRUE)
  all_files <- split(all_files, ceiling(seq_along(all_files) / 10^5))
  
  for(i in 1:length(all_files)){
    list_of_lists <- mclapply(all_files[[i]], 
                              attempt_to_open, mc.cores = cores)
    
    # Simulations that fail end up with NULL in elements 2 and 3
    ...
    tibble(id = filename) %>%
      ...
      saveRDS(paste("data/results_", i, ".rds", sep = ""))
    
    # Get allele frequencies, store in a named list
    allele_freqs <- mclapply(completed_successfully, # allele freqs for every generation
                             function(i) list_of_lists[[i]][[2]],
                             mc.cores = cores) 
    names(allele_freqs) <- filename
    saveRDS(allele_freqs, paste("data/allele_freqs_", i, ".rds", sep = ""))
    rm(allele_freqs)
    
    # Get the final population, store in a named list
    final_pop <- mclapply(completed_successfully, # population in the final generation
                          function(i) list_of_lists[[i]][[3]],
                          mc.cores = cores) 
    names(final_pop) <- filename
    saveRDS(final_pop, paste("data/final_pop_", i, ".rds", sep = ""))
    rm(final_pop)
  }
  # Zip the parameters and results together and delete the individual files
  # results_files <- paste("data/results_", 1:length(all_files), ".rds", sep = "")
  # lapply(results_files, readRDS) %>% do.call("bind_rows", .) %>%
  #   saveRDS("data/all_results.rds")
  # # unlink(results_files)
}
```
