...
make_multiplot <- function(dat){
  dat <- lapply(variable_parameters, find_percent_extinct, dataset = dat, number_of_bins = 91)
  dat <- data_for_fig1[order(sapply(dat, function(x) max(x$percent_extinct)), decreasing = TRUE)]
...
fig1 <- make_multiplot(W_shredder)
```


```{r}
fig2 <- make_multiplot(female_sterilising)
```
...
plan("multicore")
data_for_fig1_2_way <- future_lapply(1:nrow(combos), function(i) find_percent_extinct_two_way(combos[i,1], combos[i,2], W_shredder, number_of_bins = 91))
maxes <- data_for_fig1_2_way %>% map_dbl(~ max(.x$percent_extinct))
data_for_fig1_2_way <- data_for_fig1_2_way[order(maxes, decreasing = TRUE)]
...
plot_run <- function(){
 ...
 #   print(paras); print(last_generation)
  ...
  rbind(
    rbind(get_data("10024119427447", allele_freqs, W_shredder, "A. Z* causes rapid extinction") %>% 
      filter(generation >= 45), # Trim off most of the burn-in so we can see the interesting part
      data.frame(generation =61, allele = "N", frequency = 0, facet = "A. Z* causes rapid extinction")), # Add an observation of the pop size dropping to zero (not saved by the model because it only saves the info up until the generation before extinction happened)
