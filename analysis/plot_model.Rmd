...
# Command to upload the results file to Spartan
...
cairo_pdf(file = "figures/figure1.pdf", width = 10, height = 6); fig1; dev.off()
```
...
## Plot % runs ending in extinction
...
# Same as previous, only calculated separately for the 3 levels of Z_conversion_rate (male gene drive)
...
# % exinct, calculated separately for the three levels of Z_conversion_rate
...
# Function to make Figure 2 (and similar supplementary figures)
make_figure_2 <- function(dat, ylab){
...
fig2 <- make_figure_2(W_shredder_split_by_male_drive_percent_extinct, 
                      ylab = "% simulation runs resulting in extinction (\u00B1 95% CIs)")
```


```{r}
get_percent_Zd_fixed <- function(dat, parameter){
  dat %>%
    group_by(!! sym(parameter)) %>%
    summarise(yes = sum(outcome == "Zd fixed without extinction"),
              no = n() - sum(outcome == "Zd fixed without extinction"),
              prob = list(binom.test(yes, yes + no))) %>%
    rowwise() %>%
    mutate(response_variable = 100 * yes/(yes + no),
           lower_95_CI = 100 * prob$conf.int[1],
           upper_95_CI = 100 * prob$conf.int[2],
           parameter =  parameter) %>% rename(value = !! sym(parameter)) %>%
    select(parameter, value, everything()) %>% select(-prob) %>% ungroup()
...
W_shredder_split_by_male_drive_percent_Zd_fixed <- split_by_male_drive(W_shredder, "get_percent_Zd_fixed")

make_figure_2(W_shredder_split_by_male_drive_percent_Zd_fixed, 
              ylab = "% runs in which Z* fixed without causing extinction (\u00B1 95% quantiles)") + 
...
### % extinction plot for a W-shredder (Figure 2)
```{r}
cairo_pdf(file = "figures/figure2.pdf", width = 9, height = 12)
fig2 + facet_wrap(~parameter, scales = "free", labeller = label_parsed, ncol = 4) + theme(legend.position = c(.9, .1))
dev.off()
...
```{r}
S1_fig <- make_figure_2(filter(W_shredder_split_by_male_drive_time_to_extinct, n >= 40), 
...
```{r fig.width = 10.3, fig.height = 13}
S2_fig <- make_figure_2(female_sterilising_split_by_male_drive_percent_extinct,
                        ylab = "% simulation runs resulting in extinction (\u00B1 95% CIs)") + 
...
```{r}
S3_fig <- make_figure_2(filter(female_sterilising_split_by_male_drive_time_to_extinct, n >= 40), 
                        ylab = "Median generations until extinction (\u00B1 95% quantiles)") + 
  facet_wrap(~parameter, scales = "free", labeller = label_parsed, ncol = 3) + 
