...
data_for_plot <- lapply(variable_parameters, find_percent_extinct, dataset = W_shredder, number_of_bins = 91)
data_for_plot <- data_for_plot[order(sapply(data_for_plot, function(x) max(x$percent_extinct)), decreasing = TRUE)]
...
plan("multicore")
data_for_plot_2_way <- future_lapply(1:nrow(combos), function(i) find_percent_extinct_two_way(combos[i,1], combos[i,2], W_shredder, number_of_bins = 91))
maxes <- data_for_plot_2_way %>% map_dbl(~ max(.x$percent_extinct))
data_for_plot_2_way <- data_for_plot_2_way[order(maxes, decreasing = TRUE)]
