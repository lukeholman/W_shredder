...
variable_parameters <- sapply(results %>% select(-id, -realisations, -generation_extinct, -generation_Zd_extinct, -generation_W_extinct, -generation_Zd_fixed, -outcome, -allele_freqs, -final_pop, -went_extinct, -mating_table), function(x) length(unique(x))) %>% keep(~.x > 1) %>% names()
...
plot_run <- function(row, results){
  ...
  alleles_to_plot <- group_by(df, allele) %>% 
    summarise(uniques = length(unique(frequency))) %>% filter(uniques > 1) %>% .$allele
  df$frequency[df$allele == "N"] <- df$frequency[df$allele == "N"] / max(df$frequency[df$allele == "N"])
  print(tail(df,12))
