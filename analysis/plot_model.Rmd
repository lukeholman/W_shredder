...
# left_join(parameters[1:1000, ], 
#           results[1:1000, ] %>% select(-id, -mating_table, -generation_extinct, -generation_Zd_extinct, -generation_W_extinct, -generation_Zd_fixed))


if(!file.exists("data/all_results.rds")){
  files <- list.files(path = file.path(getwd(), "data"), pattern = "results_", full.names = TRUE)
  results <- lapply(1:length(files), function(i) {
...
### Time-to-extinction plot for a female-sterilising Z-drive (Figure SX
)```{r}
